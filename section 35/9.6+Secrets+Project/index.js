...
app.get("/secrets", async (req, res) => {
  ...
  if (req.isAuthenticated()) {
    try{
      const result = await db.query("SELECT * FROM users_login_info WHERE id = $1", [userId]);
...
app.post("/submit", async (req, res) => {
  ...
  const userId = req.user.id;
  try{
    await db.query("UPDATE users_login_info SET secret = $1 WHERE id = $2;", [secret, userId]);
    res.redirect("/secrets");
  }catch(err){
    console.log(err);
  }
      ...
      try {
        ...
        if (result.rows.length === 0) {
          const newUser = await db.query(
            "INSERT INTO users_login_info (email, password) VALUES ($1, $2)",
